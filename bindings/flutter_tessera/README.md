...
them at app-build time. The `publish.sh` script (repo root) builds both — SDL3
from the vendored `third_party/SDL` source, so headers match the linked binaries
exactly — and installs them into the plugin's `native/` dir, which is where the
plugin's build manifests look by default:

```sh
./publish.sh --targets macos    # -> native/macos/{libtessera,libSDL3}.dylib (universal)
./publish.sh --targets ios      # -> native/{ios-device,ios-simulator}/*.a + native/xcframeworks/
./publish.sh                    # both
```

`native/` is gitignored (regenerated by the script, never committed).

The macOS plugin's Swift Package manifest
(`macos/flutter_tessera/Package.swift`) links both universal dylibs from
`native/macos` and takes SDL3 headers from the vendored source. It bakes an
absolute rpath to `native/macos` so the app finds the two dylibs at runtime.
Override the lib dir with `TESSERA_MACOS_LIB_DIR` (or headers with
`TESSERA_INCLUDE_DIR`). A CocoaPods podspec mirrors this for non-SPM projects.
For a release app you must bundle `libtessera.dylib` and `libSDL3.dylib` into the
app's `Frameworks` and set the rpath accordingly (and disable App Sandbox during
development, since SDL opens a GPU device / window).

## Building the native library for iOS / Android

- **iOS**: `./publish.sh --targets ios` cross-compiles `libtessera` (device +
  simulator, the simulator slice universal arm64 + x86_64) and SDL3 for iOS from
  the vendored source, and installs the archives into `native/ios-device`,
  `native/ios-simulator` and `native/xcframeworks`. The engine is built with
  `TESSERA_BUILD_SHARED` off but the public `tessera_*` symbols keep default
  visibility for Dart's FFI `dlsym`. The Swift Package manifest
  (`ios/flutter_tessera/Package.swift`) is the primary build path; it links
  `libtessera.a`, `libtessera_thirdparty.a` and `libSDL3.a` from
  `native/ios-simulator` via `-force_load` plus SDL's iOS system frameworks
  (override paths with `TESSERA_IOS_LIB_DIR` / `SDL3_IOS_LIB_DIR` /
  `SDL3_INCLUDE_DIR` / `TESSERA_INCLUDE_DIR`; point them at `native/ios-device`
  for a device build), and bundles the MSL shaders as SwiftPM resources
  (`Bundle.module/shaders/`). The `ios/flutter_tessera.podspec` mirrors it for
  CocoaPods projects, bundling the shaders as `tessera_assets.bundle/shaders/`;
  both share one source tree under `ios/flutter_tessera/Sources/`. Same Metal
  path as macOS.
